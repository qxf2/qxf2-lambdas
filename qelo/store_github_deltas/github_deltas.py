...
def calculate_repo_deltas():
    ...
    previous_day_substreams = df.read_substreams(os.environ["GITHUB_SUBSTREAMS_TABLE"], yesterday)
    current_day_substreams = df.read_substreams(os.environ["GITHUB_SUBSTREAMS_TABLE"], today)
    yesterday_repos_stats = df.read_github_table(\
                os.environ["GITHUB_STATS_TABLE"], yesterday, previous_day_substreams)
    today_repos_stats = df.read_github_table(\
                os.environ["GITHUB_STATS_TABLE"], today, current_day_substreams)
...
def store_repo_deltas(repos_deltas):
    ...
    df.write_into_table(repos_deltas, os.environ["GITHUB_DELTAS_TABLE"])
...
def lambda_handler(event, context):
    ...
    response = {
                'statusCode': 200,
                'body': json.dumps({
                    'message' : 'github-deltas lambda executed successully!',
                    'deltas_status_flag' : 1})
            }
    return response
