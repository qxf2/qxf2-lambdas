...
def calculate_repo_deltas():
    ...
    previous_day_substreams = common_func.read_substreams(conf.GITHUB_SUBSTREAMS_TABLE, yesterday)
    current_day_substreams = common_func.read_substreams(conf.GITHUB_SUBSTREAMS_TABLE, today)
    yesterday_repos_stats = common_func.read_github_table(\
                conf.GITHUB_STATS_TABLE, yesterday, previous_day_substreams)
    today_repos_stats = common_func.read_github_table(\
                conf.GITHUB_STATS_TABLE, today, current_day_substreams)
...
def store_repo_deltas(repos_deltas):
    ...
    common_func.write_into_table(repos_deltas, conf.GITHUB_DELTAS_TABLE)
