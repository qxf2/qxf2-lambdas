...
def calculate_repo_deltas():
    ...
    previous_day_substreams = scf.read_substreams(conf.GITHUB_SUBSTREAMS_TABLE, yesterday)
    current_day_substreams = scf.read_substreams(conf.GITHUB_SUBSTREAMS_TABLE, today)
...
def handler(event, context):
    ...
    deltas = calculate_repo_deltas()
    message = scf.write_into_table(deltas, conf.GITHUB_DELTAS_TABLE)
    if message == 'data collected in github_deltas table':
        return "github deltas collected successfully"
    else:
